...
def populate_initial_data():
    ...
    try:
        response = requests.get(url, params=params, timeout=180) # 3 perces timeout
        ...
        conn = sqlite3.connect(DB_FILE)
        # 'replace' helyett 'append'-et használunk, és az UNIQUE constraint kezeli a duplikátumokat
...
if __name__ == '__main__':
    # Csak akkor futtatjuk a teljes telepítést, ha az adatbázis fájl még nem létezik, vagy nagyon kicsi (üres).
    if not os.path.exists(DB_FILE) or os.path.getsize(DB_FILE) < 1024: # 1KB-nál kisebb
